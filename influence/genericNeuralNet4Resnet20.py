...
def normalize_vector(v):
    ...
    """
    norm_val = np.linalg.norm(np.concatenate([x.flatten() for x in v]))
...
class GenericNeuralNet(object):
    ...
    def get_vec_to_list_fn(self):
        ...
        self.num_params = len(np.concatenate([p.flatten() for p in params_val]))        
    ...
    def print_model_eval(self):
        ...
        # Fixed: Flatten arrays before computing norms
        try:
            grad_norm = np.linalg.norm(np.concatenate([g.flatten() for g in grad_loss_val]))
        except Exception:
            grad_norm = float('nan')
            print('Warning: gradient norm skipped due to shape mismatch.')

        try:
            param_norm = np.linalg.norm(np.concatenate([p.flatten() for p in params_val]))
        except Exception:
            param_norm = float('nan')
            print('Warning: parameter norm skipped due to shape mismatch.')

        print('Norm of the mean of gradients: %s' % grad_norm)
        print('Norm of the params: %s' % param_norm)
        ...
        for i in range(num_samples):
            ...
            for j in range(recursion_depth):
                ...
                if (j % print_iter == 0) or (j == recursion_depth - 1):
                    print("Recursion at depth %s: norm is %.8lf" % (j, np.linalg.norm(np.concatenate([x.flatten() for x in cur_estimate]))))
                    feed_dict = self.update_feed_dict_with_v_placeholder(feed_dict, cur_estimate)
    ...
    def get_fmin_loss_fn(self, v):
        ...
        def get_fmin_loss(x):
            ...
            return 0.5 * np.dot(np.concatenate([h.flatten() for h in hessian_vector_val]), x) - np.dot(np.concatenate([vv.flatten() for vv in v]), x)
    ...
    def get_fmin_grad_fn(self, v):
        def get_fmin_grad(x):
            ...
            return np.concatenate([h.flatten() for h in hessian_vector_val]) - np.concatenate([vv.flatten() for vv in v])
    ...
    def get_fmin_hvp(self, x, p):
        ...
        return np.concatenate([h.flatten() for h in hessian_vector_val])
    ...
    def get_cg_callback(self, v, verbose):
        ...
        def cg_callback(x):
            # x is current params
            v_list = self.vec_to_list(x)
            idx_to_remove = 5

            single_train_feed_dict = self.fill_feed_dict_with_one_ex(self.data_sets.train, idx_to_remove)      
            train_grad_loss_val = self.sess.run(self.grad_total_loss_op, feed_dict=single_train_feed_dict)
            predicted_loss_diff = np.dot(np.concatenate([vv.flatten() for vv in v_list]), np.concatenate([g.flatten() for g in train_grad_loss_val])) / self.num_train_examples
    ...
    def get_inverse_hvp_cg(self, v, verbose):
        ...
        fmin_results = fmin_ncg(
            f=fmin_loss_fn,
            x0=np.concatenate([vv.flatten() for vv in v]),
            fprime=fmin_grad_fn,
    ...
    def get_influence_on_test_loss(self, test_indices, train_idx, 
        ...
        print('Norm of test gradient: %s' % np.linalg.norm(np.concatenate([g.flatten() for g in test_grad_loss_no_reg_val])))
        ...
        if train_idx is None:
            ...
            for counter in np.arange(num_to_remove):
                ...
                train_grad_loss_val = self.sess.run(self.grad_total_loss_op, feed_dict=single_train_feed_dict)
                predicted_loss_diffs[counter] = np.dot(np.concatenate([h.flatten() for h in inverse_hvp]), np.concatenate([g.flatten() for g in train_grad_loss_val])) / self.num_train_examples            
    ...
    def find_eigvals_of_hessian(self, num_iter=100, num_prints=10):
        ...
        for i in range(num_iter):          
            ...
            if i % print_iterations == 0:
                ...
                dotp = np.dot(np.concatenate([x.flatten() for x in new_cur_estimate]), np.concatenate([y.flatten() for y in cur_estimate]))
                print("dot: %s" % dotp)
    ...
    def get_grad_of_influence_wrt_input(self, train_indices, test_indices, 
        ...
        if verbose: print('Norm of test gradient: %s' % np.linalg.norm(np.concatenate([g.flatten() for g in test_grad_loss_no_reg_val])))
